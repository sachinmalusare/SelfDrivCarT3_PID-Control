...
int main()
{
  ...
  pid.Init(0.12, 0.0032, 0.48);
//pid.Init(0.1, 0.003, 0.5);
  ...
  h.onMessage([&pid](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
	  ...
	  pid.UpdateError(cte);
          ...
          * [-1, 1].
          * NOTE: Feel free to play around wit
h the throttle and speed. Maybe use
	  ...
	  std::cout<<cte<<" "<<steer_value<<" "<<pid.p_error<<" "<<pid.d_error<<" "<<pid.i_error<<std::endl;
	  
	  json msgJson;
          msgJson["steering_angle"] = steer_value;
          msgJson["throttle"] = 0.3;
          auto msg = "42[\"steer\"," + msgJson.dump() + "]";
          //std::myfile << msg << std::endl;
